...
;(function($) {
    ...
    $.extend(this, {
        ...
        init: function() {
            ...
            $(this).find(image_tag).css('display', 'none');
            // end

            // set all images element attribute loaded to false and hide, bcoz the
            // game is not yet started :)
            $(this).find(image_tag).attr('loaded', 'false');
            // end

            // mark first & last images
            $(this).find(image_tag).first().attr('first', 'true').css({'margin-left': '5px'});
            $(this).find(image_tag).last().attr('last', 'true').css({'margin-right': '6px'});
            // end
            ...
            // First Image
            $(this).find(image_tag).first().addClass('active');

            if (portfolio.lightbox) {
                $(gallery).find(image_tag).not('.active').animate({opacity: '0.2'});
            ...
            $(this).find(image_tag).on('movestart', function(e) {
            ...
            /* Click */
            $(this).find(image_tag).click(function(event) {
            ...
            $(this).scroll(function() {
                    if ($(gallery).find(image_tag).last().attr('loaded') === 'true') {
                        $('.gallery-blank-space').css({left: $(gallery).find(image_tag).last().data('offset-left') + $(gallery).find(image_tag).last().width() + 'px'});
                    }
                    ...
                    if ((gallery[0].offsetWidth + gallery.scrollLeft())*100 / gallery[0].scrollWidth > 60) {
                        ...
                        if (totalLoaded < $(gallery).find(image_tag).length) {
            ...
            $(window).resize(function() {
                if ($(window).width() <= 700 && $(gallery).find(image_tag).first().height()!==200) {
                    $(gallery).css({height: '200px'});
                    $(gallery).find(image_tag).css({height: '200px'});
                    $(gallery).find('.gallery-arrow-left, .gallery-arrow-right').css({height: '200px'});
                }
                else if ($(window).width() > 700 && $(gallery).find(image_tag).first().height()===200) {
                    $(gallery).css({height: portfolio.height});
                    $(gallery).find(image_tag).css({height: portfolio.height});
        ...
        next: function() {
            ...
            if($(cur_img).attr('last') === 'true') {
                ...
                if(portfolio.loop) {
                    ...
                    $(gallery).find(image_tag).removeClass('active').first().addClass('active');
            ...
            // if next image is already loaded
                ...
                $(gallery).find(image_tag).removeClass('active');
    ...
    if (this.enableKeyboardNavigation) {
            $(document).keydown(function(e) {
                    ...
                    switch(key) {
                            case 73: // 'i' key
                                        // go to first slide
                                    portfolio.slideTo($(gallery).find(image_tag).first());
                                    break;
                            case 65: // 'a' key
                                        // go to last slide
                                    portfolio.slideTo($(gallery).find(image_tag).last());
                                    break;
